...
CONFIRMED_SCRIPT="${ROOT_DIR}/scripts/dev/woocommerce-smoke-confirmed.sh"
...
if [[ ! -x "${CONFIRMED_SCRIPT}" ]]; then
  echo "Missing executable smoke test: ${CONFIRMED_SCRIPT}" >&2
  exit 1
fi

echo "Creating fresh EuroLedger WooCommerce order for confirmed E2E smoke test..." >&2
...
echo "Running confirmed smoke test for order ${ORDER_ID} / intent ${PAYMENT_INTENT_ID}..." >&2
"${CONFIRMED_SCRIPT}"

echo "Confirmed E2E WooCommerce smoke test completed successfully." >&2
