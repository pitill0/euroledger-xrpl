...
CANCELLATION_REASON="${CANCELLATION_REASON:-Customer changed their mind}"
...
$reason = $order->get_meta("_euroledger_payment_intent_cancellation_reason");

echo "ORDER_ID=" . $order->get_id() . PHP_EOL;
echo "ORDER_STATUS=" . $status . PHP_EOL;
echo "EUROLEDGER_STATUS=" . $euroledger_status . PHP_EOL;
echo "LAST_EVENT=" . $event . PHP_EOL;
echo "CANCELLATION_REASON=" . $reason . PHP_EOL;

if ("cancelled" !== $status) {
    echo "Expected WooCommerce order status cancelled" . PHP_EOL;
    exit(1);
}

if ("cancelled" !== $euroledger_status) {
    echo "Expected EuroLedger status cancelled" . PHP_EOL;
    exit(1);
}

if ("payment_intent.cancelled" !== $event) {
    echo "Expected last event payment_intent.cancelled" . PHP_EOL;
    exit(1);
}

if ("" === $reason) {
    echo "Expected cancellation reason" . PHP_EOL;
    exit(1);
}
'
}
...
reason_json="$(printf '%s' "${CANCELLATION_REASON}" | json_escape)"

echo "Cancelling payment intent ${PAYMENT_INTENT_ID} for order ${ORDER_ID}..."
cancel_response="$(api_post_json "${BACKEND_URL}/payment-intents/${PAYMENT_INTENT_ID}/cancel" "{\"reason\":${reason_json}}")"
printf '%s\n' "${cancel_response}" | python -m json.tool

status="$(printf '%s\n' "${cancel_response}" | json_field status)"
if [[ "${status}" != "cancelled" ]]; then
  echo "Expected payment intent status cancelled, got ${status}" >&2
  exit 1
fi
...
echo "Cancelled smoke test passed for order ${ORDER_ID}."
